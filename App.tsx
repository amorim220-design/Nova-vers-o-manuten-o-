...
async function openExternalUrl(url: string) {
  // ✅ Não deixa quebrar caso o plugin não exista
  try {
    const mod = await import("@capacitor/browser");
    await mod.Browser.open({ url });
  } catch {
    // fallback (web / caso plugin não esteja instalado)
...
const App: React.FC = () => {
  ...
  // ✅ Painel de erro na tela (mata a “tela branca”)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          const cleanedData = cleanLoadedData(docSnap.data());
          setData(cleanedData);
        } else {
      ...
      (error) => {
        ...
        setFatalError(`Firestore: ${String(error)}`);
        setIsLoading(false);
      }
    );
  ...
  useEffect(() => {
    const checkForUpdates = async () => {
      ...
      try {
        // ⚠️ TROQUE PELO SEU REPO CERTO (usuário + nome)
        // Ex: https://raw.githubusercontent.com/amorim220-design/manuten-ao/main/remote-version.json
  ...
  const handleUpdateConfirm = async () => {
    if (latestVersionInfo?.url) {
      await openExternalUrl(latestVersionInfo.url);
  ...
  const handleSaveName = () => {
    ...
    if (finalName && finalName !== data.userName) setData((prev) => ({ ...prev, userName: finalName }));
            ...
            h.id === hotelId
              ? {
                  ...
                  apartments: h.apartments.map((a) =>
                    a.id === apartmentId ? { ...a, items: a.items.filter((i) => i.id !== itemId) } : a
                  ),
                }
              : h
          ),
  ...
  // ✅ Telas de carregamento / login / erro
  if (fatalError) {
    return (
      <div style={{ padding: 18, color: "#ff4d4f", background: "#0b0b0b", minHeight: "100vh" }}>
        <h2 style={{ marginBottom: 12 }}>Erro no App (tela branca)</h2>
        <pre style={{ whiteSpace: "pre-wrap" }}>{fatalError}</pre>
        <p style={{ marginTop: 12, color: "#bbb" }}>
          Copie esse erro e me mande aqui que eu te digo exatamente o que corrigir.
        </p>
      </div>
    );
  }
