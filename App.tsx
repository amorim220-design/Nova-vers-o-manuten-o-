...
type View = 'dashboard' | 'hotelList' | 'hotelView' | 'apartmentView' | 'scheduleList' | 'charts' | 'billing' | 'settings';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true); // Set loading to true when user changes or first loads
  ...
  useEffect(() => {
    if (isLoading || isInitialLoad.current || !user) {
        return;
    }
  ...
  useEffect(() => {
    const checkForUpdates = async () => {
        // Update check is only for Android platform
        if (Capacitor.getPlatform() !== 'android') {
            return;
        }

        try {
            // Corrige a URL para apontar para o repositório correto: 'manuten-ao'
            const remoteVersionUrl = 'https://raw.githubusercontent.com/amorim2/manuten-ao/main/remote-version.json';
            
            const [remoteResponse, localResponse] = await Promise.all([
              fetch(`${remoteVersionUrl}?t=${new Date().getTime()}`), // Adiciona parâmetro para evitar cache
              fetch('/version.json')
            ]);
            
            if (!remoteResponse.ok || !localResponse.ok) {
                 console.error('Failed to fetch version info');
                 return;
            }

            const remoteVersionInfo = await remoteResponse.json();
            const localVersionInfo = await localResponse.json();
            
            const currentVersionCode = localVersionInfo.versionCode;
            const latestVersionCode = remoteVersionInfo.versionCode;
            ...
            if (latestVersionCode > currentVersionCode) {
                ...
                // Check if user skipped this version recently
                if (lastSkippedVersion === remoteVersionInfo.version && Date.now() - lastSkippedTime < twentyFourHours) {
                    console.log(`Update to ${remoteVersionInfo.version} was recently skipped.`);
                    return;
                }
  ...
  const handleUpdateConfirm = () => {
      if (latestVersionInfo?.url) {
          // This will open the device's default browser to download the APK
          window.open(latestVersionInfo.url, '_system');
  ...
  const handleDeleteHotel = (hotelId: string) => {
    ...
    setConfirmModalConfig({
        ...
        message: 'Esta ação é permanente. Todos os apartamentos e registros de manutenção associados a este hotel serão perdidos.',
        ...
        onConfirm: () => {
            setData(prev => ({
                ...
                hotels: prev.hotels.map(h =>
                    h.id === hotelId
                        ? { ...h, apartments: h.apartments.filter(a => a.id !== apartmentId) }
                        : h
                ),
            }));
            setConfirmModalConfig(null);
  ...
  const handleDeleteItemPhoto = (hotelId: string, apartmentId: string, itemId: string, photoIndex: number) => {
    const hotel = data.hotels.find(h => h.id === hotelId);
    const apartment = hotel?.apartments.find(a => a.id === apartmentId);
    const item = apartment?.items.find(i => i.id === itemId);
    if (!item) return;
  ...
  const renderView = () => {
    switch (view) {
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={handleSaveName} className="bg-primary-600 text-white px-3 py-1 rounded-md text-sm font-semibold hover:bg-primary-700">Salvar</button>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Olá, {data.userName}!</h1>
                  <button onClick={() => { setTempName(data.userName); setIsEditingName(true); }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 p-1 rounded-full">
                    <PencilIcon />
                  </button>
                </div>
              )}
      ...
      case 'hotelView':
        if (!selectedHotel) return <div className="p-4">Hotel não encontrado. <button onClick={handleBack} className="text-primary-500">Voltar</button></div>;
        return (
          <HotelView
            hotel={selectedHotel}
            onBack={handleBack}
      ...
      case 'apartmentView':
        if (!selectedHotel || !selectedApartment) return <div className="p-4">Apartamento não encontrado. <button onClick={handleBack} className="text-primary-500">Voltar</button></div>;
        return (
          <ApartmentView
            ...
            onDeleteItemPhoto={(itemId, photoIndex) => handleDeleteItemPhoto(selectedHotel.id, selectedApartment.id, itemId, photoIndex)}
          />
        );
      case 'scheduleList':
        return (
        ...
        <div className="container mx-auto max-w-2xl h-full flex flex-col bg-white dark:bg-slate-900 shadow-2xl relative">
          ...
          <Modal isOpen={isAddHotelModalOpen} onClose={() => setIsAddHotelModalOpen(false)} title="Adicionar Novo Hotel">
            <div className="space-y-4">
              <div>
                  <label htmlFor="hotelName" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome do Hotel</label>
                  <input type="text" id="hotelName" value={newHotelName} onChange={(e) => setNewHotelName(e.target.value)} className="input" placeholder="Ex: Hotel Palace"/>
              </div>
              <div>
                  <label htmlFor="hotelAddress" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Endereço</label>
                  <input type="text" id="hotelAddress" value={newHotelAddress} onChange={(e) => setNewHotelAddress(e.target.value)} className="input" placeholder="Ex: Av. Principal, 123"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Foto Principal (opcional)</label>
                <label htmlFor="hotel-photo-upload" className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md cursor-pointer hover:border-primary-400 dark:hover:border-primary-500">
              ...
              <div className="flex justify-end pt-2"><button onClick={handleAddHotel} className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg shadow-md">Salvar Hotel</button></div>
            </div>
          </Modal>
          ...
          <Modal isOpen={isEditHotelModalOpen} onClose={() => setIsEditHotelModalOpen(false)} title="Editar Hotel">
            <div className="space-y-4">
              <div>
                  <label htmlFor="editHotelName" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome do Hotel</label>
                  <input type="text" id="editHotelName" value={editedHotelName} onChange={(e) => setEditedHotelName(e.target.value)} className="input" />
              </div>
              <div>
                  <label htmlFor="editHotelAddress" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Endereço</label>
                  <input type="text" id="editHotelAddress" value={editedHotelAddress} onChange={(e) => setEditedHotelAddress(e.target.value)} className="input" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Foto Principal (opcional)</label>
                <label htmlFor="edit-hotel-photo-upload" className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md cursor-pointer hover:border-primary-400 dark:hover:border-primary-500">
                    <div className="space-y-1 text-center">
                    <CameraIcon />
                    <p className="text-xs text-gray-500 dark:text-gray-500">Clique para enviar ou alterar</p>
                    </div>
                    <input id="edit-hotel-photo-upload" type="file" accept="image/*" className="sr-only" onChange={handleEditedHotelPhotoUpload} />
                </label>
                {editedHotelPhoto && (
                    <div className="mt-2 relative w-32 h-32">
              ...
              <div className="flex justify-end pt-2"><button onClick={handleUpdateHotel} className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg shadow-md">Salvar Alterações</button></div>
            </div>
          </Modal>
          ...
          <Modal isOpen={isEditApartmentModalOpen} onClose={() => setIsEditApartmentModalOpen(false)} title="Editar Apartamento">
            <div className="space-y-4">
                <div>
                    <label htmlFor="editApartmentNumber" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Número do Apartamento</label>
                    <input type="text" id="editApartmentNumber" value={editedApartmentNumber} onChange={(e) => setEditedApartmentNumber(e.target.value)} className="input" />
                </div>
                 <div>
                    <label htmlFor="editApartmentDescription" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Descrição (opcional)</label>
                    <textarea id="editApartmentDescription" value={editedApartmentDescription} onChange={(e) => setEditedApartmentDescription(e.target.value)} rows={2} className="input" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Fotos (opcional, máx 5)</label>
                    <label htmlFor="edit-apt-photo-upload" className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md cursor-pointer hover:border-primary-400 dark:hover:border-primary-500">
                      ...
                      <div className="mt-2 grid grid-cols-3 sm:grid-cols-5 gap-2">
                        {editedApartmentPhotos.map((photo, index) => (
                          <div key={index} className="relative">
                            <button onClick={() => handlePhotoClick(editedApartmentPhotos, index)} className="w-full h-full block focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 rounded-md">
                                <img src={photo} alt={`Preview ${index}`} className="rounded-md object-cover h-20 w-full" />
                            </button>
                            <button onClick={() => removeEditedAptPhoto(index)} className="absolute top-0 right-0 text-red-500 bg-white bg-opacity-70 rounded-full -mt-1 -mr-1 p-0.5">
                ...
                <div className="flex justify-end pt-2"><button onClick={handleSaveEditedApartment} className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg shadow-md">Salvar Alterações</button></div>
            </div>
          </Modal>
          ...
          <Modal isOpen={isAddTaskModalOpen} onClose={() => setIsAddTaskModalOpen(false)} title="Adicionar Nova Tarefa">
            <div className="space-y-4">
              <div>
                  <label htmlFor="taskTitle" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Título</label>
                  <input type="text" id="taskTitle" value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)} className="input" placeholder="Ex: Limpar filtro do ar condicionado"/>
              </div>
               <div>
                  <label htmlFor="taskDesc" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Descrição (opcional)</label>
                  <textarea id="taskDesc" value={newTaskDescription} onChange={e => setNewTaskDescription(e.target.value)} rows={2} className="input" placeholder="Ex: Apto 101, verificar também a tubulação"/>
              </div>
              <div>
                  <label htmlFor="taskDueDate" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Data de Vencimento</label>
                  <input type="date" id="taskDueDate" value={newTaskDueDate} onChange={e => setNewTaskDueDate(e.target.value)} className="input"/>
              </div>
              <div>
                  <label htmlFor="taskPriority" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Prioridade</label>
                  <select id="taskPriority" value={newTaskPriority} onChange={e => setNewTaskPriority(e.target.value as TaskPriority)} className="input">
                    {Object.values(TaskPriority).map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
              </div>
              <div className="flex justify-end pt-2"><button onClick={handleAddTask} className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-2 px-4 rounded-lg shadow-md">Salvar Tarefa</button></div>
            </div>
          </Modal>
        </div>
      </div>
